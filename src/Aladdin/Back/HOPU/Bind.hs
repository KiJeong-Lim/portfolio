module Aladdin.Back.HOPU.Bind where
...
bind :: GenUniqueM m => LogicVar -> TermNode -> [TermNode] -> Int -> StateT Labeling (ExceptT HopuFail m) (LVarSubst, TermNode)
bind var = go . rewrite HNF where
    go :: GenUniqueM m => TermNode -> [TermNode] -> Int -> StateT Labeling (ExceptT HopuFail m) (LVarSubst, TermNode)
