...
genLexer xblocks = do
    ...
    fmap (strcat . snd) $ runWriterT $ do
        ...
        tellLine (strstr "        , getFinalQsOfDFA = XMap.fromList [" . ppunc ", " [ strstr "(" . showsPrec 0 q . strstr ", " . showsPrec 0 p . strstr ")" | (q, p) <- Map.toAscList (getFinalQsOfDFA theDFA) ] . strstr "]")
        tellLine (strstr "        , getTransitionsOfDFA = XMap.fromList " . plist 12 [ ppunc ", " [ strstr "((" . showsPrec 0 q . strstr ", " . showsPrec 0 ch . strstr "), " . showsPrec 0 p . strstr ")" | ((q, ch), p) <- deltas ] | deltas <- split' (\x1 -> \x2 -> fst (fst x1) == fst (fst x2)) (Map.toAscList (getTransitionsOfDFA theDFA)) ])
        tellLine (strstr "        , getMarkedQsOfDFA = XMap.fromList " . plist 12 [ strstr "(" . showsPrec 0 q . strstr ", XSet.fromList [" . ppunc ", " [ showsPrec 0 p | p <- Set.toAscList ps ] . strstr "])" | (q, ps) <- Map.toAscList (getMarkedQsOfDFA theDFA) ])
        tellLine (strstr "        }")
