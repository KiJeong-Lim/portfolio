...
genLexer xblocks = do
    ...
    fmap (strcat . snd) $ runWriterT $ do
        ...
        tellLine (strstr "        , getTransitionsOfDFA = XMap.fromList " . plist 8 [ ppunc ", " [ strstr "((" . showsPrec 0 q . strstr ", " . showsPrec 0 ch . strstr "), " . showsPrec 0 p . strstr ")" | ((q, ch), p) <- deltas ] | deltas <- split' (\x1 -> \x2 -> fst (fst x1) == fst (fst x2)) (Map.toAscList (getTransitionsOfDFA theDFA)) ])
        tellLine (strstr "        , getMarkedQsOfDFA = XMap.fromList " . plist 8 [ strstr "(" . showsPrec 0 q . strstr ", XSet.fromList [" . ppunc ", " [ showsPrec 0 p | p <- Set.toAscList ps ] . strstr "])" | (q, ps) <- Map.toAscList (getMarkedQsOfDFA theDFA) ])
        tellLine (strstr "        }")
